...
public class InitData implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        for (DelayTask delayTask : delayTasks) {
            if (delayTask.getClassId() == TaskClassEnum.ORDER_TASK.getCode()) {
                ...
                queue.DelayTask task = JsonUtil.jsonToObject(delayTask.getTask(), new TypeReference<queue.DelayTask<OrderTask>>() {
                });
    ...
    public static void main(String[] args) throws Exception {
        ...
        order.setId(1L);
        order.setStatus(0);
        queue.DelayTask<OrderTask> task = new queue.DelayTask<>(result, order, 1L);
